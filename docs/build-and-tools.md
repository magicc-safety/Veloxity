...
| `cargo xtask flash-board <board>` | Runs the selected board's `veloxity` binary through Cargo's runner. Pixracer Pro uses a release mode, with UART comms by default. use --vcp for usb comms on the pixracerpro |
...
Once it is confirmed that the intended board is attached, each board can be flashed/run through the configured Cargo runner:
...
The opt-in flags may be combined. Note that modifications to the runner can be performed on a per-board basis as crates own their own runner configurations.
