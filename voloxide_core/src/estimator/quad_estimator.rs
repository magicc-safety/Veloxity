...
impl QuadEstimator {
    pub fn new(k_p: f64, k_i: f64) -> Self {
        Self {
            ...
            w1: Vector::from_array([0.0, 0.0, 0.0]),
            w2: Vector::from_array([0.0, 0.0, 0.0]),
            q_extatt: None,
...
impl QuadEstimator {
    pub fn reset_state(&mut self) {
        self.q_hat = Quaternion::from_array([1.0, 0.0, 0.0, 0.0]);
        self.q_dot = Quaternion::from_array([0.0, 0.0, 0.0, 0.0]);
        self.body_rate = Vector::from_array([0.0, 0.0, 0.0]);
        self.b_hat = Vector::from_array([0.0, 0.0, 0.0]);
        self.accel_lpf = Vector::from_array([0.0, 0.0, -G]);
        self.gyro_lpf = Vector::from_array([0.0, 0.0, 0.0]);
        self.w1 = Vector::from_array([0.0, 0.0, 0.0]);
        self.w2 = Vector::from_array([0.0, 0.0, 0.0]);
        self.q_extatt = None;
    ...
    fn estimate_packets(
        ...
        if let Some(imu_packet) = imu {
            ...
            let mut kp = 0.0;
            let mut ki = self.k_i;
            let mut w_err = Vector::from_array([0.0, 0.0, 0.0]);
    ...
    fn integrate_angular_rate(&mut self, omega: Vector<f64, 3>, dt: f64, use_mat_exp: bool) {
        ...
        self.q_dot = Quaternion::from_array([
            0.5 * (-p * current.get_x() - q * current.get_y() - r * current.get_z()),
            0.5 * (p * current.get_w() + r * current.get_y() - q * current.get_z()),
            0.5 * (q * current.get_w() - r * current.get_x() + p * current.get_z()),
            0.5 * (r * current.get_w() + q * current.get_x() - p * current.get_y()),
        ]);
        ...
        if use_mat_exp {
            ...
            self.q_hat = Quaternion::from_array([
                t1 * current.get_w()
                    + t2 * (-p * current.get_x() - q * current.get_y() - r * current.get_z()),
                t1 * current.get_x()
                    + t2 * (p * current.get_w() + r * current.get_y() - q * current.get_z()),
                t1 * current.get_y()
                    + t2 * (q * current.get_w() - r * current.get_x() + p * current.get_z()),
                t1 * current.get_z()
                    + t2 * (r * current.get_w() + q * current.get_x() - p * current.get_y()),
            ]);
        } else {
...
fn accel_correction(attitude: Quaternion<f64>, accel_lpf: Vector<f64, 3>) -> Vector<f64, 3> {
    let accel_norm = accel_lpf.norm_2();
    if accel_norm <= 1e-9 {
        return Vector::from_array([0.0, 0.0, 0.0]);
    }

    let a = accel_lpf / accel_norm;
    let q_acc_inv = quaternion_between_vectors(Vector::from_array([0.0, 0.0, -1.0]), a);
    let q_tilde = q_acc_inv * attitude;
    Vector::from_array([
        -2.0 * q_tilde.get_w() * q_tilde.get_x(),
        -2.0 * q_tilde.get_w() * q_tilde.get_y(),
        0.0,
    ])
}
...
fn quaternion_between_vectors(from: Vector<f64, 3>, to: Vector<f64, 3>) -> Quaternion<f64> {
    ...
    let mut q = if dot < -0.999_999 {
        Quaternion::from_array([0.0, 1.0, 0.0, 0.0])
    } else {
        Quaternion::from_array([1.0 + dot, cross[0], cross[1], cross[2]])
    };
    q.normalize_fill();
    q
}
...
fn extatt_correction(attitude: Quaternion<f64>, external: Quaternion<f64>) -> Vector<f64, 3> {
    ...
    xext.cross3(&xhat) + yext.cross3(&yhat) + zext.cross3(&zhat)
}
